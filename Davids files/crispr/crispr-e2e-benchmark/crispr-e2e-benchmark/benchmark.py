...
def specificity_score(guide: str, target: str) -> float:
    """Deterministic test oracle: PAM-proximal mismatches carry more cost."""
    if len(guide) != len(target):
        return max(0.0, 1.0 - 0.22 * edit_distance(guide, target))
    penalty = 0.0
    for i, (a, b) in enumerate(zip(guide, target)):
        if a != b:
            penalty += 0.12 if i >= 12 else 0.06
